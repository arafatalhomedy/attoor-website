...
export default function AdminEditProject() {
    ...
    const fetchProject = async () => {
        ...
        setForm(projectData);
        setExistingGallery((mediaData || []).filter((m) => m.media_type === "gallery"));
        setExistingStages((mediaData || []).filter((m) => m.media_type === "stage"));
        setLoading(false);
    };
    ...
    const addStageRow = () => {
        setNewStages([
            ...newStages,
            { file: null, preview: null, stage_date: "", stage_title_en: "", stage_title_ar: "" },
        ]);
    };
    ...
    return (
        <div className="min-h-screen bg-cream">
            ...
            <form onSubmit={handleSave} className="max-w-3xl mx-auto px-6 py-10 space-y-8">
                <div>
                    ...
                    {newGalleryPreviews.length > 0 && (
                        <div className="grid grid-cols-3 gap-3 mb-3">
                            {newGalleryPreviews.map((src, i) => (
                                <img key={i} src={src} alt="" className="w-full h-24 object-cover rounded-md" />
                            ))}
                        </div>
                    )}
                    <input type="file" accept="image/*" multiple onChange={handleNewGallerySelect} />
                </div>
                ...
                <div>
                    ...
                    {existingStages.length === 0 ? (
                        <p className="text-sm text-zinc-500 mb-3">No timeline stages yet.</p>
                    ) : (
                        <div className="space-y-3 mb-4">
                            {existingStages.map((stage) => (
                                <div key={stage.id} className="flex items-center gap-3 bg-white border border-zinc-200 rounded-md p-3">
                                    <img src={stage.image_url} alt="" className="w-14 h-14 object-cover rounded-md" />
                                    <div className="flex-1">
                                        <p className="text-xs font-semibold text-gold">{stage.stage_date}</p>
                                        <p className="text-sm text-charcoal font-medium">{stage.stage_title_en}</p>
                                    </div>
                                    <button
                                        type="button"
                ...
                <div>
                    ...
                    {newStages.map((stage, index) => (
                        <div key={index} className="border border-zinc-200 rounded-md p-4 mb-3 space-y-3">
                            <div className="flex items-center justify-between">
                                <span className="text-sm font-semibold text-zinc-500">Stage {index + 1}</span>
                                <button
                                    type="button"
                            ...
                            {stage.preview && (
                                <img src={stage.preview} alt="" className="w-24 h-24 object-cover rounded-md" />
                            )}
                            <input
                                type="file"
                                accept="image/*"
                            ...
                            <div className="grid sm:grid-cols-3 gap-3">
                                <input
                                    ...
                                    onChange={(e) => updateStageField(index, "stage_date", e.target.value)}
                                ...
                                <input
                                    ...
                                    onChange={(e) => updateStageField(index, "stage_title_en", e.target.value)}
                                    className="rounded-md border border-zinc-300 px-3 py-2 text-sm"
                                />
                                <input
                                    type="text"
                                    ...
                                    onChange={(e) => updateStageField(index, "stage_title_ar", e.target.value)}
                                    className="rounded-md border border-zinc-300 px-3 py-2 text-sm"
                                />
                            </div>
                        </div>
